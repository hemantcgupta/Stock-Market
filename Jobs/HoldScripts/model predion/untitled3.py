...
# Separate the first row as unseen data
first_row = df.iloc[0]
...
y_encoded_remaining = label_encoder.fit_transform(y_remaining)

# Split the remaining data into training and test sets
X_train, X_test, y_train, y_test = train_test_split(X_scaled_remaining, y_encoded_remaining, test_size=0.3, random_state=42)
...
# df_updated.to_csv('updated_predictions.csv', index=False)
# Alternatively, you can insert the updated DataFrame into a database
# Data_Inserting_Into_DB(df_updated, table_name='predictions_table')
df1 = df_updated
