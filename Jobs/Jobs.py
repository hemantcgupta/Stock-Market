from growwData.growwDayMaster import *
from growwData.growwIntervalMaster import *
...
if __name__ == "__main__":
    resultJob1_1 = JobGrowwInfoDataDownloader()
    resultJob1_2 = JobGrowwInfoDataDownloaderInterval()
    resultJob2 = JobmkDayAnalyzer() # 45s Replace
    resultJob3 = JobmkMonthAnalyzer() # 15s  replace
    resultJob4 = JobmkDayProbability() # 5.20m
    resultJob5 = JobmkDayMa() # 2.20m
    resultJob6_1, resultJob6_2  = JobmkIntervalAnalyzer() # 40m
...
df = pd.DataFrame(resultJob1_1.get('db_results'))
df.groupby('status') ['tableName'].apply(list).reset_index().iloc[1]['tableName']
len(df.groupby('status') ['tableName'].apply(list).reset_index().iloc[0]['tableName'])
