        ...
        def wrapper(*args, **kwargs) -> Any:
            for i in range(1, retries + 1):
                try:
                    ...
                    filename, lineno, func_name, code = tb[-1]  # last frame where error occurred

                    if i == retries:
                        print(f'Error: {repr(e)}')
                        print(f'Failure in {filename}, line {lineno}: {code}')
                        print(f'"{func.__name__}()" failed after {retries} retries.')
                        break
                    else:
                        print(f'Error: {repr(e)}')
                        print(f'Issue at {filename}, line {lineno}: {code} -> Retrying...')
                        sleep(delay)
        return wrapper
