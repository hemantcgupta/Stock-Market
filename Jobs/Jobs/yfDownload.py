...
def period_decision_maker_for_interval_download(startDateInterval, current_date):
    ...
    period = {
        ...
        'endInterval': (current_date + timedelta(days=1)).strftime('%Y-%m-%d')
     }
    return period
    
def yfDownload(tickerName, period):
    yfTicker = yf.Ticker(f'{tickerName}.NS')
    df = yfTicker.history(period='1mo').dropna().reset_index()
...
def stockDataDownload(tickerName):
    ...
    try:
        startDate = (pd.read_sql(query, cnxn('stockmarketday')).iloc[0].iloc[0] + timedelta(days=1)).strftime('%Y-%m-%d')
    except:
        startDate = '2018-01-01'
    startDateInterval = (pd.read_sql(query, cnxn('stockmarketinterval')).iloc[0].iloc[0] + timedelta(days=1)).strftime('%Y-%m-%d')
    period = period_decision_maker(startDate, startDateInterval)
    df, dfInterval = yfDownload(tickerName, period)
    result = MK_Data_insert(df, 'stockmarketday', tickerName, 'append')
    resultInterval = MK_Data_insert(dfInterval, 'stockmarketinterval', tickerName, 'append')
