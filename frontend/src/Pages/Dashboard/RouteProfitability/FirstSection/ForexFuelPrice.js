...
class ForexFuelPrice extends Component {
    constructor(props) {
        super();
        this.state = {
            loadFactorData: [],
    ...
    getGraphData(props) {
        ...
        if (self.state.headerName === 'FUEL PRICE(USD)') {
            this.setState({ isLoading: true, loadFactorData: [] })
            apiServices.getForexFuelBarChart(startDate, endDate, routeGroup, regionId, countryId, routeId, selectedTrend, 'Fuel').then((loadFactorData) => {
                this.setState({ isLoading: false })
                if (loadFactorData) {
                    if (loadFactorData.length > 0) {
                        let graphData = [...loadFactorData[0], ...loadFactorData[1]]
                        if (!Data.isEmptyGraphData(graphData)) {
                            this.setState(
                                { loadFactorData: Data.normalizeLineChartData(graphData, selectedDelta) },
                                () => Chart.drawLineChart(this.state.loadFactorData, this.state.colors, 'loadFactorL', 'loadFactor'));
                        }
                    }
                }
            });
            ...
            this.setState({ isLoading: true, loadFactorData: [] })
            apiServices.getForexFuelBarChart(startDate, endDate, routeGroup, regionId, countryId, routeId, selectedTrend, 'Forex').then((loadFactorData) => {
                this.setState({ isLoading: false })
                if (loadFactorData) {
                    if (loadFactorData.length > 0) {
                        let graphData = [...loadFactorData[0], ...loadFactorData[1]]
        ...
        return (
            <div className="x_panel tile" style={{ paddingBottom: '15px' }}>
                <div className="x_title reduce-margin">
                    {/* <h2 className='responsive-size'>FUEL PRICE(USD)</h2> */}
                    <select class="select header-dropdown responsive-size" onChange={(e) => this.headerChange(e)}>
                        <option>FOREX(MYR/USD)</option>
                        <option>FUEL PRICE(USD)</option>
                    </select>
