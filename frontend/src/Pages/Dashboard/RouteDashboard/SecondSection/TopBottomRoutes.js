...
class TopBottomRoutes extends Component {
  ...
  getRouteData(props) {
    ...
    if (this.state.headerName === 'BOTTOM 5 ROUTES') {
      ...
      apiServices.getRouteTable(startDate, endDate, routeGroup, regionId, countryId, routeId, 'asc').then(function (result) {
      ...
      self.setState({ loading: true, routesData: [] })
      apiServices.getRouteTable(startDate, endDate, routeGroup, regionId, countryId, routeId, 'desc').then(function (result) {
        self.setState({ loading: false })
        if (result) {
          var columnName = result[0].columnName;
          var rowData = result[0].rowData;
...
export default TopBottomRoutes;