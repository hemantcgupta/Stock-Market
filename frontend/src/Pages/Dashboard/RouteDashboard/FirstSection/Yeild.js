...
class Yeild extends Component {
    ...
    getGraphData(props) {
        ...
        apiServices.getYeildBarChart(a.join('-'), endDate, routeGroup, regionId, countryId, routeId, selectedTrend, selectedDelta).then((yeildData) => {
            this.setState({ isLoading: false })
            if (yeildData) {
                if (yeildData[0].length > 0) {
                    let graphData = [...yeildData[0], ...yeildData[1]]
                    if (graphData.length > 0) {
                        this.setState(
                            { yeildData: Data.rectifiedDeltaData(graphData, selectedDelta) },
                            () => Chart.drawLineChart(this.state.yeildData, this.state.colors, 'yeildL', 'yeild', selectedDelta));
                    }
                }
            }
        });
    ...
    //     d3.select("#yeildGraph").remove();

    //     var svg = d3.select("#yeild").append("svg")
        ...
        return (
            <div className="x_panel tile" style={{ paddingBottom: '7px' }}>
                <div className="x_title reduce-margin">
                    <h2 className='responsive-size'>YIELD</h2>
