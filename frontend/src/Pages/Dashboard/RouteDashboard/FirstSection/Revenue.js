...
class Revenue extends Component {
    ...
    getGraphData(props) {
        ...
        const { selectedTrend, selectedDelta, calculatedBy } = this.state;
        ...
        apiServices.getRevenueBarChart(a.join('-'), endDate, routeGroup, regionId, countryId, routeId, selectedTrend, selectedDelta).then((revenueData) => {
            this.setState({ isLoading: false })
            if (revenueData) {
                if (revenueData[0].length > 0) {
                    let graphData = [...revenueData[0], ...revenueData[1]]
                    if (graphData.length > 0) {
                        this.setState(
                            { revenueData: Data.rectifiedDeltaData(graphData, selectedDelta) },
                            () => Chart.drawLineChart(this.state.revenueData, this.state.colors, 'revenueL', 'revenue', selectedDelta));
                    }
                }
            }
        });
    ...
    calculatedByChange = (e) => {
        e.stopPropagation();
        this.setState({ calculatedBy: e.target.value }, () => { this.getGraphData(this.props) })
    }
    ...
    //     d3.select("#revenueGraph").remove();

    //     var svg = d3.select("#revenue").append("svg")
    //         .attr("viewBox", "0 0 " + (width + margin.left + margin.right) + " " + (height + margin.top + margin.bottom))
    //         .attr("preserveAspectRatio", "none")
    //         .attr("id", "revenueGraph")
        ...
        return (
            <div className="x_panel tile">
                <div className="x_title reduce-margin">
                    <h2 className='responsive-size'>REVENUE</h2>
                    <ul className="nav navbar-right panel_toolbox">
                        {/* <label className='label'>View As:</label>
                        <select class="select" onChange={(e) => this.calculatedByChange(e)}>
