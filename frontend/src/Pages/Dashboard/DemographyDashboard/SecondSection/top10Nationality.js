...
class TopNationality extends Component {
  ...
  componentWillReceiveProps = (props) => {
    ...
    self.setState({ loading: true, NationalityData: [] })
    apiServices.getNationalityTableItems(startDate, endDate, routeGroup, regionId, countryId, cityId, routeId, posDashboard).then(function (result) {
      self.setState({ loading: false })
      if (result) {
        var columnName = result[0].columnName;
        var NationalityData = result[0].ancillaryDetails;
        var totalData = result[0].totalData;
        self.setState({ NationalityData: NationalityData, NationalityColumn: columnName, totalData: totalData })
      }
    });
    ...
    return (
      <div>
        ...
        <div className="x_content">
          <DataTableComponent
            rowData={this.state.NationalityData}
            columnDefs={this.state.NationalityColumn}
            dashboard={true}
